...
TcpServer::TcpServer(EventLoop* loop, 
    ...
    , started_(0)
{
    /* 当有新用户连接时，会执行TcpServer::newConnection的回调 */
...
void TcpServer::newConnection(int sockfd, const InetAddress& peerAddr){
    ...
    {
        LOG_ERROR("sockers::getlocalError");
    }
    InetAddress localAddr(peeraddr);

    /* 根据连接成功的sockfd的sockfd，创建Tcpconnection连接对象 */
...
TcpServer::~TcpServer(){
    ...
    {
        /* 这个局部的shared_ptr智能指针对象，出括号后，可以自动释放new出来的TcpConnection的资源 */
